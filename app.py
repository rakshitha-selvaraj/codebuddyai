...
HS_KEY   = os.environ['HINDSIGHT_KEY']
HS_BANK  = os.environ['HINDSIGHT_BANK']
HS_URL   = os.environ.get('HINDSIGHT_URL', 'https://api.hindsight.vectorize.io')

groq_client = Groq(api_key=GROQ_KEY)

# ── In-memory user store (persists while server runs) ─────────────────────────
otp_store = {}
DB_FILE   = 'users.json'
...
def register_send_otp():
    ...
    otp_store[email] = {"otp": otp, "expires_at": time.time() + 300, "type": "register"}
...
def register_complete():
    ...
    print(f"✅ New user registered and saved: {email}")
    ...
    return jsonify({"success": True, "level": level, "email": email})
            ...
            for line in memory.split('\n'):
                if 'USER_ACCOUNT' in line and f'email:{email}' in line:
                    try:
                        ...
                        user = {
                            'password_hash': parts.get('hash', ''),
                            'level': parts.get('level', 'Beginner')
                        }
...
Be warm, specific, constructive. Max 200 words."""
    ...
    mem = (
        f"Student {email} is {level} level. Practiced {topic} ({difficulty}) in {language}. "
        f"Score: {score}/100. "
        f"{'Struggled significantly — needs more practice here.' if score < 50 else 'Made some errors.' if score < 70 else 'Did well on this topic.'} "
        f"Feedback summary: {feedback[:300]}"
    )
    hs_retain(email, mem)
    ...
    return jsonify({
        "feedback": feedback,
        "score": score,
        "should_suggest_level_up": score >= 70 and difficulty == 'Hard'
    })
...
def send_feedback():
    ...
    if not team_emails:
        return jsonify({"error": "No team email addresses configured"}), 500
